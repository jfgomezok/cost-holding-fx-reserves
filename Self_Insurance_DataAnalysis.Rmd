...
```{r ESTADISTICOS, message=FALSE, warning=FALSE, include=FALSE}
...
```{r TABLA DEUDA ARGENTINA, message=FALSE, warning=FALSE, include=FALSE}

base.paper %>%
  filter( wbcode3 == "ARG", date >= "2010-01-01" ) %>%
  filter (months(date) == "diciembre") %>%
  select ( year, public_debt, PuD_Y) %>%
...
```{r definicion de periodos, echo=FALSE, message=FALSE, warning=FALSE}


all_period <- subset (base.paper, year>=2000 &  year!=2008 &  year!=2009 & year<=2017
                      & rating>3
                      & spread< 1000
                      & !wbcode3 %in% c("LBN", "CHN")
                      & lreserves_1 != "NA" &  lpublicdebt != "NA")

pre_crisis <- subset (base.paper, year>=2000 & year<=2007  
                          & rating>3 
                          & spread< 1000
                          & !wbcode3 %in% c("LBN", "CHN")
                          & lreserves_1 != "NA" & lpublicdebt != "NA")

post_crisis <- subset (base.paper, year>=2010 
                        & rating>3 
                        & spread< 1000
                        & !wbcode3 %in% c("LBN", "CHN")
                        & lreserves_1 != "NA" & lpublicdebt != "NA")
...
debt_ratio <- base.paper %>%
  ...
  filter( months(date, abbreviate=TRUE) == "dic" ) %>%
  filter( date > "2013-01-01" ) %>%
  mutate(D_Y = round(D_Y *100,1)) %>%
  mutate(date= format.Date(date, "%b-%y"))
